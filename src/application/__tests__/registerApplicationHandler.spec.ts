...
it('POST /api/v1/applications returns 400 when request body is undefined', async () => {
  const ctx = await runHandler(undefined);
  expect(ctx.status).toBe(400);
  expect(ctx.body).toEqual({ error: 'missing wooviAppId' });
});

it('POST /api/v1/applications returns 400 when request body is null', async () => {
  const ctx = await runHandler(null);
  expect(ctx.status).toBe(400);
  expect(ctx.body).toEqual({ error: 'missing wooviAppId' });
});

it('POST /api/v1/applications returns 400 when body is an empty object', async () => {
  const ctx = await runHandler({});
  expect(ctx.status).toBe(400);
  expect(ctx.body).toEqual({ error: 'missing wooviAppId' });
});

it('POST /api/v1/applications returns 400 when wooviAppId is an empty string', async () => {
  const ctx = await runHandler({ ...validBody(), wooviAppId: '' });
  expect(ctx.status).toBe(400);
  expect(ctx.body).toEqual({ error: 'missing wooviAppId' });
});

it('POST /api/v1/applications returns 400 when ixcsoft block is missing', async () => {
  const ctx = await runHandler({ wooviAppId: 'app_1' });
  expect(ctx.status).toBe(400);
  expect(ctx.body).toEqual({ error: 'missing ixcsoft' });
});

it('POST /api/v1/applications returns 400 when ixcsoft.baseUrl is missing', async () => {
...
it('POST /api/v1/applications returns 400 when ixcsoft.token is missing', async () => {
  const body = validBody();
  body.ixcsoft.token = '';
  const ctx = await runHandler(body);
  expect(ctx.status).toBe(400);
...
it('POST /api/v1/applications returns 400 when baseUrl is not a valid URL', async () => {
  const body = validBody();
...
it('POST /api/v1/applications returns 400 when baseUrl uses http instead of https', async () => {
  const body = validBody();
...
it('POST /api/v1/applications returns 400 when baseUrl does not include /webservice/v1', async () => {
  const body = validBody();
...
it('POST /api/v1/applications returns 400 when ixcsoftPing fails (unreachable, bad token, IP blocked)', async () => {
...
it('POST /api/v1/applications short-circuits before touching DB or Woovi when validation fails', async () => {
  await runHandler({});
...
it('POST /api/v1/applications returns 500 when PUBLIC_BASE_URL is empty', async () => {
...
it('POST /api/v1/applications returns 409 when wooviAppId is already registered', async () => {
...
it('POST /api/v1/applications creates the application and registers the webhook with 201', async () => {
  const ctx = await runHandler(validBody());
...
it('POST /api/v1/applications composes the webhook URL with the generated applicationId when PUBLIC_BASE_URL has no trailing slash', async () => {
  config.PUBLIC_BASE_URL = 'https://public.example.com';
  const ctx = await runHandler(validBody());
  ...
  expect(webhookMock).toHaveBeenCalledWith({
    ...
    name: 'service-ixcsoft charge completed',
    event: 'OPENPIX:CHARGE_COMPLETED',
    url: `https://public.example.com/api/v1/webhooks/charges/completed/${resBody.applicationId}`,
  });
});

it('POST /api/v1/applications strips a single trailing slash from PUBLIC_BASE_URL before composing the webhook URL', async () => {
  config.PUBLIC_BASE_URL = 'https://public.example.com/';
  const ctx = await runHandler(validBody());
  const resBody = ctx.body as { applicationId: string };
  ...
  expect(callArg.url).toBe(
    `https://public.example.com/api/v1/webhooks/charges/completed/${resBody.applicationId}`,
  );
  expect(callArg.url).not.toContain('.com//');
});

it('POST /api/v1/applications persists IXCSoft config, marks the app active and reuses the generated _id', async () => {
  const ctx = await runHandler(validBody());
  const resBody = ctx.body as { applicationId: string };
...
it('POST /api/v1/applications does not persist the application when wooviCreateWebhook rejects', async () => {
  webhookMock.mockRejectedValue(new Error('woovi down'));
  await expect(runHandler(validBody())).rejects.toThrow('woovi down');
  expect(createMock).not.toHaveBeenCalled();
});

it('POST /api/v1/applications propagates database errors from ApplicationModel.create', async () => {
